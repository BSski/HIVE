...
class TestLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        template = 'Episode {0}: reward0: {1[0]:.3f}, reward1: {1[1]:.3f}, steps: {2}'
        variables = [
            episode + 1,
            logs['episode_reward'],
            logs['nb_steps'],
...
class TrainEpisodeLogger(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        self.metrics_names = self.model.metrics_names[0]
        self.metrics_names1 = self.model.metrics_names[1]
    ...
    def on_episode_begin(self, episode, logs):
        ...
        self.observations1[episode] = []
        self.rewards[episode] = []
        self.rewards1[episode] = []
        self.actions[episode] = []
        self.actions1[episode] = []
        self.metrics[episode] = []
        self.metrics1[episode] = []
    ...
    def on_episode_end(self, episode, logs):
        ...
        # Format all metrics1.
        metrics1 = np.array(self.metrics1[episode])
        metrics_template1 = ''
        metrics_variables1 = []
        with warnings.catch_warnings():
            warnings.filterwarnings('error')
            for idx, name in enumerate(self.metrics_names1):
                if idx > 0:
                    metrics_template1 += ', '
                try:
                    value = np.nanmean(metrics1[:, idx])
                    metrics_template1 += '{}: {:f}'
                except Warning:
                    value = '--'
                    metrics_template1 += '{}: {}'
                metrics_variables1 += [name, value]
        metrics_text1 = metrics_template.format(*metrics_variables1)

        nb_step_digits = str(int(np.ceil(np.log10(self.params['nb_steps']))) + 1)

        # Original:
        '''template = '{step: ' + nb_step_digits + 'd}/{nb_steps}: episode: {episode}, duration: {duration:.3f}s, episode steps: {episode_steps:3}, \
        steps per second: {sps:3.0f}, episode reward: {episode_reward:6.3f}, mean reward: {reward_mean:6.3f} [{reward_min:6.3f}, {reward_max:6.3f}], \
        mean action: {action_mean:.3f} [{action_min:.3f}, {action_max:.3f}],  {metrics}'
        '''
        # Custom:
        template = '=================================================================================\n{step: ' + nb_step_digits + 'd}/{nb_steps}: episode: {episode},  duration: {duration:.3f}s,  steps per second:{sps:3.0f}\n----------------------------------------------------------------------------\nepisode reward: {episode_reward},  mean reward:{reward_mean:6.3f} [{reward_min}, {reward_max}],  mean action: {action_mean:.2f},\n{metrics}\n----------------------------------------------------------------------------\nepisode_reward1: {episode_reward1},  mean reward1: {reward1_mean:6.3f} [{reward1_min}, {reward1_max}],  mean action1: {action1_mean:.2f},\n{metrics1}\n================================================================================='
        ...
        variables = {
            ...
            'episode_reward1': np.sum(self.rewards1[episode]),
            'reward1_mean': np.mean(self.rewards1[episode]),
            'reward1_min': np.min(self.rewards1[episode]),
            'reward1_max': np.max(self.rewards1[episode]),
            'action1_mean': np.mean(self.actions1[episode]),
            'metrics1': metrics_text1
        }
        print(template.format(**variables))
        ...
        del self.observations1[episode]
        del self.rewards1[episode]
        del self.actions1[episode]
        del self.metrics1[episode]
    ...
    def on_step_end(self, step, logs):
        ...
        self.observations1[episode].append(logs['observation1'])
        self.rewards1[episode].append(logs['reward1'])
        self.actions1[episode].append(logs['action1'])
        self.metrics1[episode].append(logs['metrics1'])
        self.step += 1
